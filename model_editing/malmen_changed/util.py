...
def empty_cache(path: str):
    ...
    for s in suffixes:
        ...
        if os.path.isdir(full_path):
            ...
            print(f"Removing directory and its contents: {full_path}")
...
def get_shape(module: Union[nn.Linear, Conv1D]) -> Tuple[int]:
    ...
    return shape[::-1] if isinstance(module, nn.Linear) else get_shape
...
def cross_entropy(
    ...
    if len(logits.shape) == 2:
        ...
        return F.cross_entropy_with_logits(logits, labels)

    if len(logits.shape) == 3:

        ans_indice = torch.where(labels != -100)
        print("ans_indicie", ans_indice) 
