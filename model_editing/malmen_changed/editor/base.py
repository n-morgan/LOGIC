...
class BaseEditor:
    ...
    def train(self, loader: DataLoader):
        ...
        for tuples in tqdm(loader, desc = "Train", ncols = 100):
            ...
            self.cache((tuples["edit_tuples"],tuples["embeddings"]))
            ...
            for t,embedding in zip(tuples["equiv_tuples"],tuples["embeddings"]):
                logits = self.model(**t,embeddings=embedding)["logits"]
                labels_unsqueezed = t["labels"].unsqueeze(1).to(torch.float32)

                loss = cross_entropy(logits, labels_unsqueezed)
                loss.backward()
                gen_losses += [loss.item()]
            self.edit_model(param_shifts, True)
            loc_losses = []
            for t,embedding in zip(tuples["unrel_tuples"], tuples["embeddings"]):
                with torch.no_grad():
                    refer_logits = self.model(**t, embeddings=embedding)["logits"]
                ...
                loss = kl_div(
                    refer_logits,
                    logits,
                    t["labels"].unsqueeze(1).to(torch.float32)

                ).mean()
                # print("************************* \n LOSS: " + f'{torch.min(loss)}')
                (self.config.editor.loc_coef * loss).backward()
                self.edit_model(param_shifts, True)
                loc_losses += [loss.item()]
            print("************ BASE.PY GEN and LOC LOSSES **************")
            ...
            for k, s in zip(
                ...
                for t in tuples[k]:
                    with torch.no_grad():
                        ...
                        logits = self.model(**t, embeddings=tuples["embeddings"])["logits"]
    ...
    def cache(self, tuples: List[Dict[str, torch.LongTensor]]):
        ...
        reg, embeddings = tuples
        for idx, (t, embedding) in enumerate(zip(reg,embeddings)):
            ...
            with TracerDict(
                ...
                print ('******************* BASE.PY, CACHE_FN ******************')


                labels_unsqueezed = t["labels"].unsqueeze(1).to(torch.float32)
                cross_entropy(logits, labels_unsqueezed).backward()
